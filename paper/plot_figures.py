...
def plot_metric(metric_name, out_name, title, ylabel):
    ...
    for f in glob.glob("wandb_export_*.csv"):
        ...
        for col in df.columns:
            ...
            # Parse col name for PickCube only
            if "PickCube" not in col:
                continue
    ...
    variants_main = [
        "tanh", 
        "tanh_sq", 
        "concave_truncated (R=0.2)", 
        "concave_truncated (R=0.1)", 
...
# Evaluate standard success once
plot_metric(" - eval/success_once", "fig", "Eval Success Once", "Eval Success Rate")
...
plot_metric(" - eval/return", "fig_eval_return", "Eval Return", "Evaluation Return")
plot_metric(" - train/return", "fig_train_return", "Train Return", "Training Return")
plot_metric(" - train/success_once", "fig_train_success", "Train Success Once", "Training Success Rate")
